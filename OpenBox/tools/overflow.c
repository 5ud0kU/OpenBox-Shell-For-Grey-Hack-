...
overflow.craft = function(index,addr,buffer,key)
  ...
  card = {"idx":str(index),"address":addr,"buffer":buffer,"key":key,"typeof":"undefined"};
  overflow.deck.push(card);

  return "deck updated with new card.";
end function
...
overflow.drop = function()
  ...
  for i in range(0,overflow.deck.len -1)
    text = text + str(i) + " " + overflow.deck[i].address + " " + overflow.deck[i].buffer + " " + overflow.deck[i].key + " " + overflow.deck[i].typeof + char(10);
  end for
